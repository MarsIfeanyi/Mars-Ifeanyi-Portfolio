...
{/* navlinks */}
<div className={`text-white ${sidebarOpenClose} fixed justify-center flex flex-col h-full transform transition-all duration-500 delay-300 w-[80%] sm:w-[60%] bg-blue-500 space-y-6 z-1050`} >
{Navlinks.map((link,index) => {
  return <Link key={index} href={link.href} >
<p className='text-white w-fit text-[20px] ml-12 border-b-1[1.5px] pb-1 border-white sm:text-[30px]' >{link.name} </p>
  </Link>
})}
